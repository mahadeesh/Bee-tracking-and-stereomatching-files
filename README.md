...
1. Despite being a semi-automatic bee tracking program, the finally generated 3D position data contain head as well as tail locations of freely flying bees, making it a unique and valuable dataset.

2. The developed stereomatching code solves the correspondence matching problem just by using two cameras.

3.	The developed tracking program can also be applied to track bees flying in other environments, such as a linear or curved flight tunnel.


## Prerequisites

All that is needed is MATLAB software from MathWorks<sup>2</sup>. The developed code has been tested using MATLAB R2020 update 5 running on a macOS Catalina. All of the function files specified in the branch must be present.


## Contents of the files

1)	*Sci_data_bee_tracking_pgm_3.m* - Main bee tracking program, which tracks individual bees flying in the bee cloud arena.
...
3)	*stereo_triangulation_camera_positions.m* – used to calculate the 3D positions of the two cameras with respect to the origin.

4)	*fit_ellipse.m* – an open-source ellipse fitting program.
...
2.	Go to ‘Image option’ in the main menu >> stacks >> Z project >> select projection type as ‘median’ and click ‘OK’ >> this will generate and image with only the background scene (variable name – ‘ov’ in the code).
...
5. Save the resulting final video using the ‘save as’ option from the main menu. Then, select ‘avi’ from the list of options to save the final video in the destination folder as chosen by the user.


## Running the bee tracking program
...
a) The tracked 2D coordinates of a bee will be saved in ‘*J\_BC\_HT\_CORDS\_DATASET\_#\_CAM\_##\_REFER\_BEE\_###.mat*’.

b) The frames numbers at which occlusions occurred will be saved in ‘*OCCUL\_FRAMES\_DATASET\_#\_CAM\_##\_REFER\_BEE\_###.mat*’.

c) The frames where a bee image appears circular in shape will be saved in ‘*RATIO\_A\_BY\_B\_DATASET\_#\_CAM\_##\_REFER\_BEE\_###.mat*’. 
...
#### Note: The developed tracking program was able to track bees flying in a bee cloud successfully. The code has also been tested by applying it to track individual bees flying in a curved tunnel. Please refer to the tracking results section to see the tracking output.

## Tracking a bee from any frame number

To track bees from any point in the video, just key in the frame number from which you would like to begin the tracking process.

Say for instance you are tracking a bee which is present in the arena from frame 1 to frame 800. Assume for some reason that Matlab is not responding after 500 frames. In that case, you might need to close and reopen Matlab again. Now, instead of recommencing from frame 1, you could continue the tracking process from frame 499. This could be done by simply updating the ‘frame_number’ and ‘frame_count’ variables with the frame number from which you would like to continue the tracking process. Also, uncomment the ‘load’ command for the 4 output variables so that the tracked information from frame 1 to frame 499 will be loaded into the program when executing the program from frame 500 onwards.
...
## Running the stereomatching program
